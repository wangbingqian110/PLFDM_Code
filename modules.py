...
class DinoFeaturizer(nn.Module):
    ...
    def __init__(self, dim, cfg):
        ...
        if self.cfg.modelName == 'DINO':
            ...
            if arch == "vit_small":
                ...
                self.n_feats = 768
        elif self.cfg.modelName == 'YOLO':
            # YOLOv5
            self.model = YOLO('yolo/yolov8n-cls.pt').model
            self.model.eval().cuda()

        for p in self.model.parameters():
            p.requires_grad = False
        self.dropout = torch.nn.Dropout(p=.1)
        # self.pool_kernel = cfg.kernel_size_feat_pool
        # self.pooldim = int(self.n_feats * self.feat_w * self.feat_w / self.pool_kernel)
        # if self.n_feats * self.feat_w * self.feat_w % self.pool_kernel != 0:
        #     raise
        self.upsample = nn.Upsample(size=192, mode='nearest')
        self.cluster1 = self.make_clusterer(3)
        self.proj_type = cfg.projection_type
        if self.proj_type == "nonlinear":
            self.cluster2 = self.make_nonlinear_clusterer(3)
        # self.codelinearLayer = self.codelinear()

    # def codelinear(self):
    #     return torch.nn.Sequential(
    #         torch.nn.Linear(self.dim * self.feat_w * self.feat_w, self.dim))

    def make_clusterer(self, in_channels):
        return torch.nn.Sequential(
            torch.nn.Linear(384, self.feature_num))  # ,

    def make_nonlinear_clusterer(self, in_channels):
        return torch.nn.Sequential(
            torch.nn.Linear(384, self.res * self.res),
            torch.nn.Tanh(),
            torch.nn.Linear(self.res * self.res, self.res * self.res),
            torch.nn.Tanh(),
            torch.nn.Linear(self.res * self.res, self.res * self.res),
            torch.nn.Tanh(),
            torch.nn.Linear(self.res * self.res, self.res * self.res),
            ...
            torch.nn.Linear(self.res * self.res, self.res * self.res),
            torch.nn.Tanh(),
            torch.nn.Linear(self.res * self.res, self.feature_num))
    ...
    def forward(self, img, img_orgin, n=1, return_class_feat=False):
        self.model.eval()
        with torch.no_grad():
        ...
        image_feat = image_feat.reshape(image_feat.shape[0], -1)
        # imgPrcessed = torch.layer_norm(imgPrcessed, normalized_shape=imgPrcessed.shape)
        # upsample_feat = self.upsample(image_feat.unsqueeze(0)).squeeze(0)
        # imgPrcessed = upsample_feat + self.upsample(img_orgin.unsqueeze(0)).squeeze(0)  # 残差链接
        ...
        # code = self.codelinearLayer(code)

        # image_feat = img.reshape(img.shape[0], -1)
        # img_orgin = self.upsample(img_orgin.unsqueeze(0)).squeeze(0)
        # code = code + self.upsample(img_orgin.unsqueeze(0)).squeeze(0)  # 残差链接
        if self.cfg.dropout:
            return self.dropout(img_orgin), code
        else:
            return img_orgin, code
...
def tensor_correlation(a, b):
    return torch.einsum("nf,nf->n", a, b)


def tensor_correlation_1D(a, b):
    # return torch.einsum("nf,nf->n", a, b)
...
def gt_sim(feat, feat_inter):
    tensor = tensor_correlation(norm(feat), norm(feat_inter))
    mask = torch.gt(tensor, 0)
...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def forward(self,
                orig_feats: torch.Tensor, orig_feats_pos: torch.Tensor, intra_feats: torch.Tensor,
                feat_neg: torch.Tensor,
                orig_code: torch.Tensor, orig_code_pos: torch.Tensor, intra_code: torch.Tensor, code_neg: torch.Tensor
                ):
        feats = orig_feats
        code = orig_code

        # intra,KNN_spetral
        pos_intra_loss, pos_intra_cd, pos_intra_feat = self.helper(
            feats, intra_feats, code, intra_code, self.cfg.pos_intra_shift)

        # inter,KNN_spatial
        orig_feats_pos = orig_feats_pos.permute(1, 0, 2)
        orig_code_pos = orig_code_pos.permute(1, 0, 2)
        pos_losses = []
        pos_cds = []
        pos_fds = []
        for i in range(orig_feats_pos.shape[0]):
            # 正类比较
            feats_pos = orig_feats_pos[i]
            code_pos = orig_code_pos[i]
            effect = gt_sim(feats, feats_pos)
            pos_inter_loss, pos_inter_cd, pos_inter_feat = self.helper(
                feats[effect], feats_pos[effect], code[effect], code_pos[effect], self.cfg.pos_inter_shift)
            pos_losses.append(pos_inter_loss)
            pos_cds.append(pos_inter_cd)
            pos_fds.append(pos_inter_feat)

        # neg
        orgin_feats_neg = feat_neg.permute(1, 0, 2)
        orgin_code_neg = code_neg.permute(1, 0, 2)

        neg_losses = []
        neg_cds = []
        neg_fds = []
        for i in range(orgin_feats_neg.shape[0]):
            # 负类比较
            feats_neg = orgin_feats_neg[i]
            code_neg = orgin_code_neg[i]
            neg_inter_loss, neg_inter_cd, neg_inter_feat = self.helper(
                feats, feats_neg, code, code_neg, self.cfg.neg_inter_shift)
            neg_losses.append(neg_inter_loss)
            neg_cds.append(neg_inter_cd)
            neg_fds.append(neg_inter_feat)
        pos_inter_loss = torch.cat(pos_losses, axis=0)
        pos_inter_cd = torch.cat(pos_cds, axis=0)
        pos_inter_feat = torch.cat(pos_fds, axis=0)
        neg_inter_loss = torch.cat(neg_losses, axis=0)
        neg_inter_cd = torch.cat(neg_cds, axis=0)
        neg_inter_feat = torch.cat(neg_fds, axis=0)

        return (pos_inter_loss,
                pos_inter_cd,
                pos_inter_feat,
                pos_intra_loss,
                pos_intra_cd,
